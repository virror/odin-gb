...
opcodes: [256]Opcode = {
    ...
    {OCDE, 2, 1, "SBC A, n"},
    {OCDF, 4, 0, "RST 0x18"},
    {OCE0, 3, 1, "LDH 0xFFn, A"},
    {OCE1, 3, 0, "POP HL"},
    {OCE2, 2, 0, "LD 0xFF00 + C, A"},
    {nil , 1, 0, "N/A"},
    {nil , 1, 0, "N/A"},
    {OCE5, 4, 0, "PUSH HL"},
    {OCE6, 2, 1, "AND A, n"},
...
OC08 :: proc() {
    ...
    case 3:
        bus_write(state.value, u8(SP))
    case 4:
        bus_write(state.value + 1, u8(SP >> 8))
    }
}
...
OC36 :: proc() {
    ...
    case 1:
        state.value = u16(cpu_fetch())
    case 2:
        bus_write(reg.HL, u8(state.value))
    }
}
...
OCE0 :: proc() {
    ...
    case 1:
        state.value = u16(cpu_fetch())
    case 2:
        bus_write(0xFF00 + state.value, reg.A)
    }
}
...
OCE8 :: proc() {
    ...
    case 3:
        orgSP := SP
        bus_dummy()
        SP = u16(i16(SP) + i16(state.value))
        reg.F.H = bool(((orgSP & 0xF) + (u16(state.value) & 0xF)) & 0x10)
        _, ovf := intrinsics.overflow_add(u8(orgSP), state.value)
        reg.F.C = ovf
    }
}
...
OCF0 :: proc() {
    ...
    case 1:
        state.value = u16(cpu_fetch())
    case 2:
        reg.A = bus_read(0xFF00 + state.value)
    }
}
...
//LD A, FF00+C
OCF2 :: proc() {
    reg.A = bus_read(0xFF00 + u16(reg.C))
}
...
INCx :: proc(index: Index) {
    ...
    if(state.cycle == 2 || state.op.cycles == 1) {
        newValue := u8(state.value + 1)
        setReg(index, newValue)
        reg.F.Z = newValue == 0
        reg.F.N = false
        cpu_setHalfAdd8(u8(state.value), 1)
    }
}
...
DECx :: proc(index: Index) {
    ...
    if(state.cycle == 2 || state.op.cycles == 1) {
        ...
        reg.F.Z = newValue == 0
        reg.F.N = true
        cpu_setHalfSub8(u8(state.value), 1)
    }
}
...
ADDx :: proc(index: Index) {
    value := getReg(index)
    newValue := reg.A + value
    reg.F.Z = newValue == 0
    reg.F.N = false
    cpu_setHalfAdd8(reg.A, value)
    cpu_setCarryAdd8(reg.A, value)
    reg.A = newValue
}
...
SUBx :: proc(index: Index) {
    ...
    reg.F.Z = newValue == 0
    reg.F.N = true
    cpu_setHalfSub8(reg.A, value)
    cpu_setCarrySub8(reg.A, value)
    reg.A = newValue
}
